...
```
/feature {기능명}      → 요구사항 정의 (템플릿 생성 또는 기존 파일 분석)
  ↓                       → 사용자가 요구사항 작성/확인
  ↓                       → 로드맵 MD 생성
  ↓ (사용자 검토)
/execute               → 로드맵 파일 확인 → 진행 상황 표시
  ↓                       → 다음 태스크 실행 (사용자 승인 후)
  ↓                       → 완료 기록
  ↓ (다음 태스크 승인)
/execute               → 반복 (태스크마다 시작/완료 승인)
  ↓
...
`/feature` 실행 시 요구사항 파일(`docs/feature/{name}.md`)이 있으면 분석, 없으면 템플릿을 생성한다.
...
| 커맨드 | 용도 | 타이밍 |
|--------|------|--------|
| `/feature {기능명}` | 요구사항 정의 + 로드맵 생성 | 프로젝트 시작 시 1회 |
| `/execute {로드맵명}` | 로드맵 기반 태스크 순차 실행 (태스크별 승인) | 세션마다 반복 |
| `/fix` | 오류 수정 (증상 수집 → 원인 분석 → 수정) | 버그 발생 시 |
| `/make-doc` | 프로젝트 문서 생성 (로드맵, 테스트 시나리오, 가이드, 보고서) | 문서 작성 시 |
...
태스크 완료 시 아래 항목을 확인하고 결과를 사용자에게 보고한다:
- 레이어드 아키텍처 준수 여부
- 타입 안전성 (Python 타입 힌트, TypeScript `any` 미사용)
- `ApiResponse[T]` 표준 응답 래퍼 사용
- DB 변경 시 Alembic 마이그레이션 포함
- 프론트 API 호출 시 `apiClient` 사용
