...
```
/sdd {메뉴명}          → 대화형 질문으로 요구사항 수집 → 로드맵 MD 생성
  ↓ (사용자 검토)
/sdd-task              → 다음 미완료 태스크 실행 → 완료 기록 + 검수 체크리스트
  ↓ (사용자 검수)
/sdd-task              → 반복 (세션마다 1 태스크)
  ↓
...
SDD 워크플로우 시작 시 사용자는 다음 5가지를 제공한다:
1. **메뉴명** — 화면/기능 이름
2. **테이블명 + DDL** — CREATE TABLE 문 또는 컬럼 목록
3. **라우터 URL** — API 및 프론트 경로
4. **요구사항** — 기능 명세 (자연어)
5. **레거시 참조** (선택) — 기존 함수/프로시저/쿼리

### 스킬 (슬래시 커맨드)

| 커맨드 | 용도 | 타이밍 |
|--------|------|--------|
| `/sdd {메뉴명}` | 로드맵 생성 (대화형 질문) | 프로젝트 시작 시 1회 |
| `/sdd-task {로드맵명} {태스크번호}` | 다음 태스크 실행 | 세션마다 반복 |
