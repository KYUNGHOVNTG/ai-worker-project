...
### Phase 1: 즉시 필요 (바이브코딩 첫 경험을 좌우)

> **Phase 1의 목표**: clone → 1~2개 명령어 → 브라우저에서 동작하는 CRUD 데모 확인 (5분 이내)

#### 1-1. sample_domain을 "진짜 동작하는 데모"로 완성
...
- Alembic 마이그레이션 생성 (**DB 마이그레이션 필요**)
- API 엔드포인트 라우터에 등록
...
**무게**: 파일 1개, 유지보수 부담 거의 없음

---

#### 1-3. DB 시드 데이터 + SQLite 폴백
...
**제안**: Makefile의 `make dev`로 통합
```bash
make dev    # 백엔드 + 프론트 동시 기동
```

**무게**: Makefile에 타겟 1개 추가, 의존성 추가 없음
...
**DB 마이그레이션 필요**: `users` 테이블 생성 (email, hashed_password, is_active 등)
**범위 제한**: 이메일+비밀번호 로그인만. OAuth는 가이드 문서로 안내.
**무게**: 도메인 1개 추가 (기존 아키텍처 그대로 활용)

---

#### 2-2. UI 컴포넌트 보강 (기존 스켈레톤 완성 + 신규)
...
| 항목 | 마이그레이션 내용 | 비고 |
|------|-----------------|------|
| 1-1 sample_domain 동작 완성 | `sample_data` 테이블 생성 | 기존 모델 기반 autogenerate |
| 2-1 Auth 도메인 | `users` 테이블 생성 | email, hashed_password, is_active 등 |
| 2-1 Auth 도메인 (선택) | `refresh_tokens` 테이블 | 토큰 블랙리스트/갱신용 |
...
```
Phase 1 — 첫 경험 (1-2일)
├── 1-1. sample_domain E2E 동작 완성 ← 가장 중요
│   └── [DB 마이그레이션: sample_data 테이블]
├── 1-2. Makefile (setup/dev/test/lint)
├── 1-3. SQLite 폴백 + 시드 스크립트
└── 1-4. 동시 실행 (make dev)

Phase 2 — 제품 골격 (1주 내)
├── 2-1. Auth 도메인 (로그인/회원가입)
│   └── [DB 마이그레이션: users 테이블]
├── 2-2. UI 컴포넌트 (기존 완성 + 신규 추가)
└── 2-3. 404/에러 페이지

Phase 3 — 품질 보강 (2주 내)
└── 3-1. 테스트 예시 보강 (기존 주석 복원 + 신규)
...
아래 항목은 이 로드맵의 범위에서 **의도적으로 제외**합니다:

| 제외 항목 | 제외 사유 |
|----------|----------|
| Docker 환경 | 회사 사용 시 Docker Desktop 유료 (250인+ 또는 $10M+ 기업). 설치 자체가 러닝커브 상승. SQLite 폴백이 더 가벼운 대안 |
| 배포 가이드 | 소프트랜딩 범위 초과. 제품화 단계에서 별도 진행 |
| CLI 스캐폴딩 | 바이브코딩에서는 Claude가 파일 생성을 대신하므로 스크립트 불필요 |
| 국제화 (i18n) | 현 단계에서 한국어만 지원하면 충분 |
| 환경별 설정 분리 | 개발 환경(.env 1개)으로 충분. 제품화 시 추가 |
| pre-commit 훅 | Claude가 코드 품질을 관리하는 바이브코딩 환경에서는 우선순위 낮음 |
...
2. **복사-붙여넣기 가능** — 모든 예시는 그대로 복사해서 동작해야 함
3. **기존 아키텍처 존중** — Layered + Domain Plugin 구조 변경 금지
4. **한국어 우선** — 문서와 주석은 한국어, 코드는 영어
5. **Supabase 선택적** — SQLite 폴백으로 외부 서비스 없이도 시작 가능
